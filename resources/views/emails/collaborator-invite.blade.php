...
<head>
    ...
    <title>Convite para Treinamento</title>
    <style>
...
<div class="wrapper">
    ...
    <div class="body">
        <div class="greeting">Olá{{ $collaborator->name ? ', ' . $collaborator->name : '' }}!</div>

        <p class="text">
            Você foi convidado por <strong>{{ $leader->name }}</strong> para participar do programa de conscientização em segurança digital da sua empresa:
        </p>

        <div class="company-badge">🏢 {{ $leader->company->name }}</div>

        <p class="text">
            O treinamento é rápido (10 a 15 minutos) e vai te preparar para identificar situações reais de <strong>golpes, phishing e engenharia social</strong> no ambiente de trabalho.
        </p>

        <div class="btn-wrap">
            <a href="{{ $magicLinkUrl }}" class="btn">Iniciar meu treinamento →</a>
        </div>

        <div class="warning">
            ⏱️ <strong>Este link é de uso único e expira em 30 dias.</strong><br>
            Não compartilhe este link. Ele é pessoal e vinculado ao seu e-mail.
        </div>
